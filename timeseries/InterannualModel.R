...
windows()
ggplot(all_before_threshold, aes(x=day, y=y_norm)) +
  geom_point(aes(color=as.factor(pheno_year), group=tree_year)) +
  geom_line(data=all_predictions, aes(x=day, y=y_norm, color=pheno_year, group=pheno_year),
            size=0.5) +
  labs(title="Cavallinesia platanifolia: MLE of interannual effects",
       y="Leaf Cover Percentage",
       x="Date",
       color="Phenological Year") +
  scale_x_continuous(
    breaks = {
      ref <- as.Date(paste0(pheno_year_levels[1], "-09-01"))
      as.numeric(seq(ref, ref + 365, by = "month") - ref)
    },
    labels = format(seq(as.Date(paste0(pheno_year_levels[1], "-09-01")),
                        as.Date(paste0(pheno_year_levels[1], "-09-01")) + 365,
                        by = "month"), "%b")
  ) +
  theme_minimal()

ggsave("plots/phenology_predictions.png", width=10, height=6)
