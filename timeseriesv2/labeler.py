...
current_labels_label = None  # UI label for current polygon labels

def format_current_labels(polygon_id):
    crown_data = saved_polygons.get('crowns', {}).get(str(polygon_id), {})
    labels = crown_data.get('labels', {})
    leafing_value = labels.get('leafing', 'not set')
    flowering_value = labels.get('flowering', 'not set')
    quality_value = crown_data.get('quality', 'not set')
    edited_value = crown_data.get('edited', False)
    no_edits_needed_value = crown_data.get('no_edits_needed', False)
    date_value = crown_data.get('date', 'unknown')

    return (
        f"Current labels for {polygon_id}:\n"
        f"Date: {date_value}\n"
        f"Leafing: {leafing_value}\n"
        f"Flowering: {flowering_value}\n"
        f"Quality: {quality_value}\n"
        f"Edited: {edited_value}\n"
        f"No edits needed: {no_edits_needed_value}"
    )

def update_current_labels(polygon_id):
    if current_labels_label is None:
        return
    current_labels_label.setText(format_current_labels(polygon_id))
...
def load_tree(polygon_id):
    ...
    try:
        ...
        update_current_labels(polygon_id)
...
def on_global_id_changed(selected_id):
    ...
    # Find first polygon that needs editing (not marked as no_edits_needed or edited)
    unedited_index = None
    for idx, row in tree_rows.iterrows():
        polygon_id = str(row['polygon_id'])
        crown_data = saved_polygons['crowns'].get(polygon_id, {})
        if not (crown_data.get('no_edits_needed', False) or crown_data.get('edited', False)):
            unedited_index = tree_rows.index.get_loc(idx)
            break
    
    # If all are marked as edited or no_edits_needed, start at index 0
    if unedited_index is None:
        unedited_index = 0
...
current_labels_label = QLabel("Current labels: (none)")
current_labels_label.setWordWrap(True)
layout.addWidget(current_labels_label)
